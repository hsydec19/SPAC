# We create 'ResidualAttentionBlock_SPAC' to achieve low memory consumption in prompt learning
...
class ResidualAttentionBlock_SPAC(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        if self.add_prompt:
            ...
            nn.init.normal_(ctx_vectors, std=0.02)
            self.VPT_shallow = nn.Parameter(ctx_vectors)
            self.teacher = ctx_vectors.clone().cuda()
    ...
    def forward(self, x, bj=None):

        if self.add_prompt and bj != 'zs':
            # Also see if this is textual transformer layer or not
            if not self.text_layer:
                if x.shape[0] == 197: # insert 197->201
                    # Remove the outputs produced by learnable tokens of previous layer
                    # Create/configure learnable tokens of this layer
                    if bj == 'tea':
                        visual_context = self.teacher.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                    else:
                        visual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                    # Add the learnable tokens of this layer with the input, by replacing the previous
                    # layer learnable tokens
                ...
                # Create/configure learnable tokens of this layer
                if bj == 'tea':
                    textual_context = self.teacher.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                else:
                    textual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                # Add the learnable tokens of this layer with the input, replaced by previous
                # layer learnable tokens
    ...
    #             # x -> [77, NCLS, DIM]
    #             # First remove the learnable tokens from previous layer
    #             self.VPT_shallow = x[1:1+self.n_ctx_text,0 , :]
    #             return x
    #             prefix = x[:1, :, :]
    #             suffix = x[1 + self.n_ctx_text:, :, :]
    #             # Create/configure learnable tokens of this layer
...
class Transformer(nn.Module):
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'SPAC':
                self.resblocks = nn.Sequential(*[ResidualAttentionBlock_SPAC(width, heads, attn_mask, False,
                                                                            text_layer, i,
                                                                            design_details) if 12-prompts_needed > i
                                                else ResidualAttentionBlock_SPAC(width, heads, attn_mask, True,
                                                                                text_layer, i, design_details)
                                                for i in range(layers)])
        else:
    ...
    def forward(self, x: torch.Tensor, bj=None):
        if self.trainer == "SPAC":
